/** Catálogo completo tal como llega del JSON; lo usa search.js para filtrar sin volver a pedirlo. */
...
function createGameCard(game) {
    ...
    cover.loading = "lazy";
    // Si la portada remota falla (sin conexión, CDN caído), se reemplaza por un recurso local propio.
    cover.addEventListener("error", () => {
        cover.onerror = null;
        cover.src = "./assets/img/no-image.svg";
    }, { once: true });
...
function loadGames() {
    ...
    return fetch("./assets/js/data/games.json")
        .then((response) => {
            if (!response.ok) throw new Error(`Error HTTP ${response.status}`);
            return response.json();
        })
        .then((games) => {
            catalogoCompleto = games;
